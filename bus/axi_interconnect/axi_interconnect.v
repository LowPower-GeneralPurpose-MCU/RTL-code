...
#(
    ...
    // parameter [0:(MST_AMT*32)-1]    MST_WEIGHT          = {32'd4, 32'd3, 32'd2, 32'd1},
    parameter [127:0]               MST_WEIGHT          = 128'h00000005_00000003_00000002_00000001,
    ...
    // Gộp [SLV3, SLV2, SLV1, SLV0]
    parameter [SLV_AMT*32-1:0] SLV_BASE_ADDR = {
        32'h5000_0000, // S3: FLASH 
        32'h8000_0000, // S2: RAM
        32'h4000_0000, // S1: APB
        32'h0000_1000  // S0: ROM
    },
    parameter [SLV_AMT*32-1:0] SLV_ADDR_MASK = {
        32'hFF00_0000, // S3: FLASH Mask (16MB)
        32'hFFFF_0000, // S2: RAM Mask (64KB)
        32'hFFFF_0000, // S1: APB Mask (64KB)
        32'hFFFF_F000  // S0: ROM Mask (4KB)
    },
...
(
    ...
    input   [MST_AMT-1:0]                   m_AWLOCK_i,
    input   [4*MST_AMT-1:0]                 m_AWCACHE_i,
    input   [3*MST_AMT-1:0]                 m_AWPROT_i,
    input   [4*MST_AMT-1:0]                 m_AWQOS_i,
    input   [4*MST_AMT-1:0]                 m_AWREGION_i,
    input   [MST_AMT-1:0]                   m_AWVALID_i,
    // Write data channel
    input   [DATA_WIDTH*MST_AMT-1:0]        m_WDATA_i,
    input   [(DATA_WIDTH/8)*MST_AMT-1:0]    m_WSTRB_i,
    ...
    input   [MST_AMT-1:0]                   m_ARLOCK_i,
    input   [4*MST_AMT-1:0]                 m_ARCACHE_i,
    input   [3*MST_AMT-1:0]                 m_ARPROT_i,
    input   [4*MST_AMT-1:0]                 m_ARQOS_i,
    input   [4*MST_AMT-1:0]                 m_ARREGION_i,
    ...
    output  [SLV_AMT-1:0]                   s_AWLOCK_o,
    output  [4*SLV_AMT-1:0]                 s_AWCACHE_o,
    output  [3*SLV_AMT-1:0]                 s_AWPROT_o,
    output  [4*SLV_AMT-1:0]                 s_AWQOS_o,
    output  [4*SLV_AMT-1:0]                 s_AWREGION_o,
    output  [SLV_AMT-1:0]                   s_AWVALID_o,
    // Write data channel
    output  [DATA_WIDTH*SLV_AMT-1:0]        s_WDATA_o,
    output  [(DATA_WIDTH/8)*SLV_AMT-1:0]    s_WSTRB_o,
    ...
    output  [SLV_AMT-1:0]                   s_ARLOCK_o,
    output  [4*SLV_AMT-1:0]                 s_ARCACHE_o,
    output  [3*SLV_AMT-1:0]                 s_ARPROT_o,
    output  [4*SLV_AMT-1:0]                 s_ARQOS_o,
    output  [4*SLV_AMT-1:0]                 s_ARREGION_o,
    ...
    wire                            m_AWLOCK    [MST_AMT-1:0];
    wire    [3:0]                   m_AWCACHE   [MST_AMT-1:0];
    wire    [2:0]                   m_AWPROT    [MST_AMT-1:0];
    wire    [3:0]                   m_AWQOS     [MST_AMT-1:0];
    wire    [3:0]                   m_AWREGION  [MST_AMT-1:0];
    wire                            m_AWVALID   [MST_AMT-1:0];
    // Write data channel
    wire    [DATA_WIDTH-1:0]        m_WDATA     [MST_AMT-1:0];
    wire    [DATA_WIDTH/8-1:0]      m_WSTRB     [MST_AMT-1:0];
    ...
    wire                            m_ARLOCK    [MST_AMT-1:0];
    wire    [3:0]                   m_ARCACHE   [MST_AMT-1:0];
    wire    [2:0]                   m_ARPROT    [MST_AMT-1:0];
    wire    [3:0]                   m_ARQOS     [MST_AMT-1:0];
    wire    [3:0]                   m_ARREGION  [MST_AMT-1:0];
    ...
    wire                            s_AWLOCK    [SLV_AMT-1:0];
    wire    [3:0]                   s_AWCACHE   [SLV_AMT-1:0];
    wire    [2:0]                   s_AWPROT    [SLV_AMT-1:0];
    wire    [3:0]                   s_AWQOS     [SLV_AMT-1:0];
    wire    [3:0]                   s_AWREGION  [SLV_AMT-1:0];
    wire                            s_AWVALID   [SLV_AMT-1:0];
    // Write data channel
    wire    [DATA_WIDTH-1:0]        s_WDATA     [SLV_AMT-1:0];
    wire    [DATA_WIDTH/8-1:0]      s_WSTRB     [SLV_AMT-1:0];
    ...
    wire                            s_ARLOCK    [SLV_AMT-1:0];
    wire    [3:0]                   s_ARCACHE   [SLV_AMT-1:0];
    wire    [2:0]                   s_ARPROT    [SLV_AMT-1:0];
    wire    [3:0]                   s_ARQOS     [SLV_AMT-1:0];
    wire    [3:0]                   s_ARREGION  [SLV_AMT-1:0];
    ...
    wire    [SLV_AMT-1:0]                   dsp_sa_AWLOCK_o         [MST_AMT-1:0];
    wire    [4*SLV_AMT-1:0]                 dsp_sa_AWCACHE_o        [MST_AMT-1:0];
    wire    [3*SLV_AMT-1:0]                 dsp_sa_AWPROT_o         [MST_AMT-1:0];
    wire    [4*SLV_AMT-1:0]                 dsp_sa_AWQOS_o          [MST_AMT-1:0];
    wire    [4*SLV_AMT-1:0]                 dsp_sa_AWREGION_o       [MST_AMT-1:0];
    ...
    wire    [(DATA_WIDTH/8)*SLV_AMT-1:0]    dsp_sa_WSTRB_o          [MST_AMT-1:0];
    ...
    wire    [SLV_AMT-1:0]                   dsp_sa_ARLOCK_o         [MST_AMT-1:0];
    wire    [4*SLV_AMT-1:0]                 dsp_sa_ARCACHE_o        [MST_AMT-1:0];
    wire    [3*SLV_AMT-1:0]                 dsp_sa_ARPROT_o         [MST_AMT-1:0];
    wire    [4*SLV_AMT-1:0]                 dsp_sa_ARQOS_o          [MST_AMT-1:0];
    wire    [4*SLV_AMT-1:0]                 dsp_sa_ARREGION_o       [MST_AMT-1:0];
    ...
    wire    [MST_AMT-1:0]                   sa_dsp_AWLOCK_i         [SLV_AMT-1:0];
    wire    [4*MST_AMT-1:0]                 sa_dsp_AWCACHE_i        [SLV_AMT-1:0];
    wire    [3*MST_AMT-1:0]                 sa_dsp_AWPROT_i         [SLV_AMT-1:0];
    wire    [4*MST_AMT-1:0]                 sa_dsp_AWQOS_i          [SLV_AMT-1:0];
    wire    [4*MST_AMT-1:0]                 sa_dsp_AWREGION_i       [SLV_AMT-1:0];
    ...
    wire    [(DATA_WIDTH/8)*MST_AMT-1:0]    sa_dsp_WSTRB_i          [SLV_AMT-1:0];
    ...
    wire    [MST_AMT-1:0]                   sa_dsp_ARLOCK_i         [SLV_AMT-1:0];
    wire    [4*MST_AMT-1:0]                 sa_dsp_ARCACHE_i        [SLV_AMT-1:0];
    wire    [3*MST_AMT-1:0]                 sa_dsp_ARPROT_i         [SLV_AMT-1:0];
    wire    [4*MST_AMT-1:0]                 sa_dsp_ARQOS_i          [SLV_AMT-1:0];
    wire    [4*MST_AMT-1:0]                 sa_dsp_ARREGION_i       [SLV_AMT-1:0];
    ...
    generate
        ...
        for(mst_idx = 0; mst_idx < MST_AMT; mst_idx = mst_idx + 1) begin : MST_DEFLAT
            ...
            assign m_AWLOCK[mst_idx]                                        = m_AWLOCK_i[mst_idx];
            assign m_AWCACHE[mst_idx]                                       = m_AWCACHE_i[4*(mst_idx+1)-1-:4];
            assign m_AWPROT[mst_idx]                                        = m_AWPROT_i[3*(mst_idx+1)-1-:3];
            assign m_AWQOS[mst_idx]                                         = m_AWQOS_i[4*(mst_idx+1)-1-:4];
            assign m_AWREGION[mst_idx]                                      = m_AWREGION_i[4*(mst_idx+1)-1-:4];
            ...
            assign m_WSTRB[mst_idx]                                         = m_WSTRB_i[(DATA_WIDTH/8)*(mst_idx+1)-1-:DATA_WIDTH/8];
            ...
            assign m_ARLOCK[mst_idx]                                        = m_ARLOCK_i[mst_idx];
            assign m_ARCACHE[mst_idx]                                       = m_ARCACHE_i[4*(mst_idx+1)-1-:4];
            assign m_ARPROT[mst_idx]                                        = m_ARPROT_i[3*(mst_idx+1)-1-:3];
            assign m_ARQOS[mst_idx]                                         = m_ARQOS_i[4*(mst_idx+1)-1-:4];
            assign m_ARREGION[mst_idx]                                      = m_ARREGION_i[4*(mst_idx+1)-1-:4];
        ...
        for(slv_idx = 0; slv_idx < SLV_AMT; slv_idx = slv_idx + 1) begin : SLV_DEFLAT
            ...
            assign s_AWLOCK_o[slv_idx]                                              = s_AWLOCK[slv_idx];
            assign s_AWCACHE_o[4*(slv_idx+1)-1-:4]                                  = s_AWCACHE[slv_idx];
            assign s_AWPROT_o[3*(slv_idx+1)-1-:3]                                   = s_AWPROT[slv_idx];
            assign s_AWQOS_o[4*(slv_idx+1)-1-:4]                                    = s_AWQOS[slv_idx];
            assign s_AWREGION_o[4*(slv_idx+1)-1-:4]                                 = s_AWREGION[slv_idx];
            ...
            assign s_WSTRB_o[(DATA_WIDTH/8)*(slv_idx+1)-1-:DATA_WIDTH/8]            = s_WSTRB[slv_idx];
            ...
            assign s_ARLOCK_o[slv_idx]                                              = s_ARLOCK[slv_idx];
            assign s_ARCACHE_o[4*(slv_idx+1)-1-:4]                                  = s_ARCACHE[slv_idx];
            assign s_ARPROT_o[3*(slv_idx+1)-1-:3]                                   = s_ARPROT[slv_idx];
            assign s_ARQOS_o[4*(slv_idx+1)-1-:4]                                    = s_ARQOS[slv_idx];
            assign s_ARREGION_o[4*(slv_idx+1)-1-:4]                                 = s_ARREGION[slv_idx];
        ...
        for(mst_idx = 0; mst_idx < MST_AMT; mst_idx = mst_idx + 1) begin : NETWORK_CONNECTION
            ...
            assign sa_dsp_AWLOCK_i[slv_idx][mst_idx]                                            = dsp_sa_AWLOCK_o[mst_idx][slv_idx];
            assign sa_dsp_AWCACHE_i[slv_idx][4*(mst_idx+1)-1-:4]                                = dsp_sa_AWCACHE_o[mst_idx][4*(slv_idx+1)-1-:4];
            assign sa_dsp_AWPROT_i[slv_idx][3*(mst_idx+1)-1-:3]                                 = dsp_sa_AWPROT_o[mst_idx][3*(slv_idx+1)-1-:3];
            assign sa_dsp_AWQOS_i[slv_idx][4*(mst_idx+1)-1-:4]                                  = dsp_sa_AWQOS_o[mst_idx][4*(slv_idx+1)-1-:4];
            assign sa_dsp_AWREGION_i[slv_idx][4*(mst_idx+1)-1-:4]                               = dsp_sa_AWREGION_o[mst_idx][4*(slv_idx+1)-1-:4];
            ...
            assign sa_dsp_WSTRB_i[slv_idx][(DATA_WIDTH/8)*(mst_idx+1)-1-:DATA_WIDTH/8]          = dsp_sa_WSTRB_o[mst_idx][(DATA_WIDTH/8)*(slv_idx+1)-1-:DATA_WIDTH/8];
            ...
            assign sa_dsp_ARLOCK_i[slv_idx][mst_idx]                                            = dsp_sa_ARLOCK_o[mst_idx][slv_idx];
            assign sa_dsp_ARCACHE_i[slv_idx][4*(mst_idx+1)-1-:4]                                = dsp_sa_ARCACHE_o[mst_idx][4*(slv_idx+1)-1-:4];
            assign sa_dsp_ARPROT_i[slv_idx][3*(mst_idx+1)-1-:3]                                 = dsp_sa_ARPROT_o[mst_idx][3*(slv_idx+1)-1-:3];
            assign sa_dsp_ARQOS_i[slv_idx][4*(mst_idx+1)-1-:4]                                  = dsp_sa_ARQOS_o[mst_idx][4*(slv_idx+1)-1-:4];
            assign sa_dsp_ARREGION_i[slv_idx][4*(mst_idx+1)-1-:4]                               = dsp_sa_ARREGION_o[mst_idx][4*(slv_idx+1)-1-:4];
    ...
    generate
        for(mst_idx = 0; mst_idx < MST_AMT; mst_idx = mst_idx + 1) begin : DSP_GEN
            ai_dispatcher #(
                ...
                .m_AWLOCK_i(m_AWLOCK[mst_idx]),
                .m_AWCACHE_i(m_AWCACHE[mst_idx]),
                .m_AWPROT_i(m_AWPROT[mst_idx]),
                .m_AWQOS_i(m_AWQOS[mst_idx]),
                .m_AWREGION_i(m_AWREGION[mst_idx]),
                .m_AWVALID_i(m_AWVALID[mst_idx]),
                .m_WDATA_i(m_WDATA[mst_idx]),
                .m_WSTRB_i(m_WSTRB[mst_idx]),
                ...
                .m_ARLOCK_i(m_ARLOCK[mst_idx]),
                .m_ARCACHE_i(m_ARCACHE[mst_idx]),
                .m_ARPROT_i(m_ARPROT[mst_idx]),
                .m_ARQOS_i(m_ARQOS[mst_idx]),
                .m_ARREGION_i(m_ARREGION[mst_idx]),
                ...
                .sa_AWLOCK_o(dsp_sa_AWLOCK_o[mst_idx]),
                .sa_AWCACHE_o(dsp_sa_AWCACHE_o[mst_idx]),
                .sa_AWPROT_o(dsp_sa_AWPROT_o[mst_idx]),
                .sa_AWQOS_o(dsp_sa_AWQOS_o[mst_idx]),
                .sa_AWREGION_o(dsp_sa_AWREGION_o[mst_idx]),
                .sa_AWVALID_o(dsp_sa_AWVALID_o[mst_idx]),
                .sa_AW_outst_full_o(dsp_sa_AW_outst_full_o[mst_idx]),
                .sa_WDATA_o(dsp_sa_WDATA_o[mst_idx]),
                .sa_WSTRB_o(dsp_sa_WSTRB_o[mst_idx]),
                ...
                .sa_ARLOCK_o(dsp_sa_ARLOCK_o[mst_idx]),
                .sa_ARCACHE_o(dsp_sa_ARCACHE_o[mst_idx]),
                .sa_ARPROT_o(dsp_sa_ARPROT_o[mst_idx]),
                .sa_ARQOS_o(dsp_sa_ARQOS_o[mst_idx]),
                .sa_ARREGION_o(dsp_sa_ARREGION_o[mst_idx]),
        ...
        for(slv_idx = 0; slv_idx < SLV_AMT; slv_idx = slv_idx + 1) begin : SA_GEN
            if(MST_AMT > 1) begin   // Multiple masters -> Need arbitrate between masters
                ai_slave_arbitration #(
                    ...
                    .TRANS_DATA_SIZE_W(TRANS_DATA_SIZE_W),
                    .TRANS_WR_RESP_W(TRANS_WR_RESP_W),
                    .SLV_BASE_ADDR(SLV_BASE_ADDR[slv_idx*32 +: 32]),
                    .SLV_ADDR_MASK(SLV_ADDR_MASK[slv_idx*32 +: 32])
                ) slave_arbitration (
                    .ACLK_i(ACLK_i),
                    .ARESETn_i(ARESETn_i),
                    ...
                    .dsp_AWLOCK_i(sa_dsp_AWLOCK_i[slv_idx]),
                    .dsp_AWCACHE_i(sa_dsp_AWCACHE_i[slv_idx]),
                    .dsp_AWPROT_i(sa_dsp_AWPROT_i[slv_idx]),
                    .dsp_AWQOS_i(sa_dsp_AWQOS_i[slv_idx]),
                    .dsp_AWREGION_i(sa_dsp_AWREGION_i[slv_idx]),
                    ...
                    .dsp_WSTRB_i(sa_dsp_WSTRB_i[slv_idx]),
                    ...
                    .dsp_ARLOCK_i(sa_dsp_ARLOCK_i[slv_idx]),
                    .dsp_ARCACHE_i(sa_dsp_ARCACHE_i[slv_idx]),
                    .dsp_ARPROT_i(sa_dsp_ARPROT_i[slv_idx]),
                    .dsp_ARQOS_i(sa_dsp_ARQOS_i[slv_idx]),
                    .dsp_ARREGION_i(sa_dsp_ARREGION_i[slv_idx]),
                    ...
                    .s_AWLOCK_o(s_AWLOCK[slv_idx]),
                    .s_AWCACHE_o(s_AWCACHE[slv_idx]),
                    .s_AWPROT_o(s_AWPROT[slv_idx]),
                    .s_AWQOS_o(s_AWQOS[slv_idx]),
                    .s_AWREGION_o(s_AWREGION[slv_idx]),
                    .s_AWVALID_o(s_AWVALID[slv_idx]),
                    .s_WDATA_o(s_WDATA[slv_idx]),
                    .s_WSTRB_o(s_WSTRB[slv_idx]),
                    ...
                    .s_ARLOCK_o(s_ARLOCK[slv_idx]),
                    .s_ARCACHE_o(s_ARCACHE[slv_idx]),
                    .s_ARPROT_o(s_ARPROT[slv_idx]),
                    .s_ARQOS_o(s_ARQOS[slv_idx]),
                    .s_ARREGION_o(s_ARREGION[slv_idx]),
                ...
                assign s_AWLOCK[slv_idx]            = sa_dsp_AWLOCK_i[slv_idx];
                assign s_AWCACHE[slv_idx]           = sa_dsp_AWCACHE_i[slv_idx];
                assign s_AWPROT[slv_idx]            = sa_dsp_AWPROT_i[slv_idx];
                assign s_AWQOS[slv_idx]             = sa_dsp_AWQOS_i[slv_idx];
                assign s_AWREGION[slv_idx]          = sa_dsp_AWREGION_i[slv_idx];
                ...
                assign s_WSTRB[slv_idx]             = sa_dsp_WSTRB_i[slv_idx];
                ...
                assign s_ARLOCK[slv_idx]            = sa_dsp_ARLOCK_i[slv_idx];
                assign s_ARCACHE[slv_idx]           = sa_dsp_ARCACHE_i[slv_idx];
                assign s_ARPROT[slv_idx]            = sa_dsp_ARPROT_i[slv_idx];
                assign s_ARQOS[slv_idx]             = sa_dsp_ARQOS_i[slv_idx];
                assign s_ARREGION[slv_idx]          = sa_dsp_ARREGION_i[slv_idx];
